...
export default async function MedicaoDetalhePage({ params }: { params: { id: string, medicaoId: string } }) {
    ...
    if (!medicao) {
        redirect(`/obras-eng/${params.id}/medicao`)
    }
    ...
    const { data: medicaoItensAtual } = await supabase
        .from('medicao_itens')
        .select('*')
        .eq('medicao_id', params.medicaoId)
