...
// Utiliza a SERVICE_ROLE se disponível para bypass RLS no backend, caso contrário tenta com a ANON_KEY
...
async function syncData() {
    ...
    try {
        ...
        console.log('Conectado ao SQL Server (UAU).');

        // Query fornecida, com filtro temporal (apenas de hoje em diante)
        const query = `
        ...
        console.log(`Retornados ${records.length} registros do SQL Server (recentes).`);
        ...
        console.log(`Sincronização concluída. ${insertedCount} novos pedidos criados no Supabase.`);
...
// Handler para finalizar graciosamente
process.on('SIGINT', () => {
...
// Inicia o loop
console.log('Worker de Integração UAU <-> Supabase iniciado.');
syncData(); // Roda a primeira vez imediatamente
