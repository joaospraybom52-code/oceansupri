...
export default function ControleClient({ obras, medicoesIniciais, podeEditar, comprometido = [], fluxoRecebido = [], fluxoPago = [], fluxoAPagar = [], fluxoAReceber = [] }: { obras: Obra[]; medicoesIniciais: Medicao[]; podeEditar: boolean; comprometido?: ComprometidoMes[]; fluxoRecebido?: FluxoDia[]; fluxoPago?: FluxoDia[]; fluxoAPagar?: FluxoDia[]; fluxoAReceber?: FluxoDia[] }) {
    ...
    return (
        <div>
            ...
            <FluxoCaixaDiario recebidos={fluxoRecebido} pagos={fluxoPago} aPagar={fluxoAPagar} aReceber={fluxoAReceber} obras={obras} />
...
//   A receber= mesma medida do painel "Próximas Medições" (valor_prc por
//              data_fim_contrato_ven em controle_a_receber).
// Filtros próprios: Mês/Ano (padrão = mês/ano atual) e Obra (com busca).
function FluxoCaixaDiario({ recebidos, pagos, aPagar = [], aReceber = [], obras }: { recebidos: FluxoDia[]; pagos: FluxoDia[]; aPagar?: FluxoDia[]; aReceber?: FluxoDia[]; obras: Obra[] }) {
    ...
    const anos = useMemo(() => {
        ...
        aReceber.forEach(r => { if (r.data) s.add(r.data.slice(0, 4)) })
        return Array.from(s).sort()
    }, [recebidos, pagos, aPagar, aReceber])
    ...
    const obraOptions = useMemo(() => {
        ...
        aReceber.forEach(r => { if (r.obra) codigos.add(r.obra) })
    ...
    }, [recebidos, pagos, aPagar, aReceber, obras])

    // Dias do mês selecionado com as somas de recebido/pago
    const { chartData, totalCred, totalDeb, totalAPagar, totalAReceber } = useMemo(() => {
        const ym = `${ano}-${mes}`
        const nDias = new Date(Number(ano), Number(mes), 0).getDate()
        const dias = Array.from({ length: nDias }, (_, i) => ({
            dia: String(i + 1).padStart(2, '0'),
            Recebido: 0, Pago: 0, 'A pagar': 0, 'A receber': 0,
        }))
        const soma = (rows: FluxoDia[], campo: 'Recebido' | 'Pago' | 'A pagar' | 'A receber') => {
            for (const r of rows) {
        ...
        soma(pagos, 'Pago')
        soma(aPagar, 'A pagar')
        soma(aReceber, 'A receber')
        return {
            chartData: dias,
            totalCred: dias.reduce((s, d) => s + d.Recebido, 0),
            totalDeb: dias.reduce((s, d) => s + d.Pago, 0),
            totalAPagar: dias.reduce((s, d) => s + d['A pagar'], 0),
            totalAReceber: dias.reduce((s, d) => s + d['A receber'], 0),
        }
    }, [recebidos, pagos, aPagar, aReceber, mes, ano, obraSel])

    const temDados = totalCred > 0 || totalDeb > 0 || totalAPagar > 0 || totalAReceber > 0
        ...
        <div className="glass-card" style={{ padding: '24px', marginTop: '24px' }}>
            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-end', justifyContent: 'space-between', gap: '16px', marginBottom: '20px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>realizado × previsto por dia</span>
                </div>
                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', alignItems: 'flex-end' }}>
                    ...
                    <div style={{ display: 'flex', gap: '18px', paddingBottom: '4px' }}>
                        ...
                        <div style={{ textAlign: 'right' }}>
                            <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>A receber</div>
                            <div style={{ fontSize: '15px', fontWeight: 800, color: '#22d3ee', whiteSpace: 'nowrap' }}>{formatCurrency(totalAReceber)}</div>
                        </div>
                        <div style={{ textAlign: 'right' }}>
                        ...
                        <div style={{ textAlign: 'right' }}>
                            ...
                            <div style={{ fontSize: '15px', fontWeight: 800, color: totalCred + totalAReceber - totalDeb - totalAPagar >= 0 ? '#10b981' : '#ef4444', whiteSpace: 'nowrap' }}>{formatCurrency(totalCred + totalAReceber - totalDeb - totalAPagar)}</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            {!temDados ? (
                ...
                <>
                        ...
                        <BarChart data={chartData} margin={{ top: 10, right: 16, left: 0, bottom: 10 }}>
                            ...
                            <Tooltip
                                ...
                                content={({ active, payload }) => {
                                    ...
                                    if (!d || (d.Recebido === 0 && d.Pago === 0 && d['A pagar'] === 0 && d['A receber'] === 0)) return null
                                    ...
                                    return (
                                        <div style={tooltipStyle}>
                                            ...
                                            <p style={{ margin: '2px 0', fontSize: '13px', color: '#22d3ee' }}>A receber: <strong style={{ color: '#f1f5f9' }}>{formatCurrency(d['A receber'])}</strong></p>
                            ...
                            <Bar dataKey="A receber" fill="#22d3ee" radius={[3, 3, 0, 0]} maxBarSize={22} isAnimationActive={false} />
                    ...
                    <div style={{ display: 'flex', justifyContent: 'center', gap: '20px', marginTop: '14px' }}>
                        ...
                        <span style={{ display: 'flex', alignItems: 'center', gap: '7px', fontSize: '12px', color: '#94a3b8' }}><span style={{ width: 12, height: 12, borderRadius: 3, background: '#22d3ee' }} /> A receber</span>
